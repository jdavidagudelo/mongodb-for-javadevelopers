...
public class UserDAO {
    ...
    public boolean addUser(String username, String password, String email) throws UnknownHostException {
        ...
        if (email != null && !email.equals("")) {
        	...
        	((BasicDBObject)user).append("email", email);
        }

        try {
            // XXX WORK HERE
            // insert the document into the user collection here
        	((BasicDBObject)user).append("_id", username);
        	((BasicDBObject)user).append("password", passwordHash);
    ...
    public DBObject validateLogin(String username, String password) {
        ...
        DBObject find = new BasicDBObject("_id", username);
        
        user = usersCollection.findOne(find);
        if (user == null) {
